...
// to move the scene around, since we can't move the camera yet
	...
	if (testTriangleIntersect(rayPos, rayDir, sphereX, vec3(1.0f, 0.0f, 5.0f), vec3(-1.0f, 1.0f, 5.0f), hitInfo))
    {
...
void main()
{
	...
	uint rngState = uint(uint(gl_FragCoord.x) * uint(1973) + uint(gl_FragCoord.y) * uint(9277) + uint(iTime) * uint(26699)) | uint(1);

	vec2 texturecoord = vec2(gl_FragCoord.xy / vec2(game_window_x, game_window_y));
	...
	vec3 rayTarget = vec3(((gl_FragCoord.xy + jitter) / vec2(game_window_x, game_window_y)) * 2.0 - 1.0, cameraDistance);
	...
	color = mix(texture, color, 1.0/float(iTime + 1.0));
